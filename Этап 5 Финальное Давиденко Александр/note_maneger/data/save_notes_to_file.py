...
def save_notes_to_file(all_info,filename): # - функция записи заметки в файл формата txt
    notes = open(filename, 'w', encoding='utf-8') # - открытие файла для написания текста с деодирование под кирилицу
    ...
    for idx, note in enumerate(all_info):
        ...
        if idx < len(all_info) - 1:
            notes.write("\n")
    print("Заметки успешно перезаписаны!")
